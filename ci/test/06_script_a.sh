...
CI_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$BGL_CONFIG_ALL" "$BGL_CONFIG" || ( (CI_EXEC cat config.log) && false)

CI_EXEC make distdir VERSION="$HOST"


CI_EXEC ./configure --cache-file=../config.cache "$BGL_CONFIG_ALL" "$BGL_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  CI_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/BGL-config.h > src/config/BGL-config.h.tmp && mv src/config/BGL-config.h.tmp src/config/BGL-config.h'
fi
