 ...
 *  -> tx7_ASFR = (high+low+high) / (tx4_vsize + tx5_vsize + tx7_vsize) */
    ...
    for (const auto& sequences : {miniminer_manual.Linearize(), miniminer_pool.Linearize()}) {
        ...
        BOOST_CHECK_EQUAL(Find(sequences, tx4->GetHash()), 0); //The selected first is at index 1(BGL)
        BOOST_CHECK_EQUAL(Find(sequences, tx2->GetHash()), 1); //The selected first is at index 0(BGL)
