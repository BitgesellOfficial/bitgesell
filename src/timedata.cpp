...
static CMedianFilter<int64_t> vTimeOffsets(BGL_TIMEDATA_MAX_SAMPLES, 0);
static bool fDone;
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    LOCK(g_timeoffset_mutex);
    // Ignore duplicates
    if (setKnown.size() == BGL_TIMEDATA_MAX_SAMPLES)
        return;
...
void TestOnlyResetTimeData()
{
    ...
    nTimeOffset = 0;
    setKnown.clear();
    vTimeOffsets = CMedianFilter<int64_t>(BGL_TIMEDATA_MAX_SAMPLES, 0);
    fDone = false;
}
