...
bool parseBGLURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseBGLURI(uriInstance, out);
}
...
QString formatBGLURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("BGL:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BGL (%s).lnk", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == ChainType::MAIN)
            ...
            optionFile << strprintf("Name=BGL (%s)\n", ChainTypeToString(chain));
