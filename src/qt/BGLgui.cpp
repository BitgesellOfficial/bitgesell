...
void BGLGUI::createActions()
{
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bitgesell command-line options").arg(CLIENT_NAME));
    ...
    if(walletFrame)
    {
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BGLGUI::setCurrentWallet, Qt::QueuedConnection);
...
void BGLGUI::createTrayIconMenu()
{
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BGLGUI::toggleHidden);
...
void BGLGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bitgesell network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BGLGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BGLGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BGLGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
