...
const int BGL_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BGL_IPC_PREFIX("BGL:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BGL_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BGL_IPC_CONNECT_TIMEOUT);
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start BGL: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("BGL://", Qt::CaseInsensitive))
    {
    ...
    else if (s.startsWith(BGL_IPC_PREFIX, Qt::CaseInsensitive)) // BGL: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBGLURI(s, &recipient))
            {
