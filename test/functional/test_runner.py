...
def main():
    ...
    # Read config generated by configure. The runner's own help should remain
    # available before configure has generated test/config.ini.
    ...
    if os.path.exists(configfile):
        ...
        parser.print_help()
        print()
        print("Per-test help is unavailable until ./configure generates test/config.ini.")
        sys.exit(0)
    else:
        parser.error(f"configuration file not found: {configfile}. Run ./configure before running functional tests.")
    ...
    # Create base test directory after all help-only paths, so --help remains side-effect-free.
