...
class ConfArgsTest(BGLTestFramework):
    ...
    def test_negated_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'BGL.conf'
        ...
        ignored_file_message = [f'[InitConfig] Data directory "{self.nodes[0].datadir_path}" contains a "BGL.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        with open(self.nodes[0].datadir_path / 'BGL.conf', 'a', encoding='utf-8') as conf:
    ...
    def test_config_file_log(self):
        ...
        node = self.nodes[0]
        conf_text = node.BGLconf.read_text()
    ...
    def test_ignored_conf(self):
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "BGL.conf" file which is ignored, because a '
    ...
    def test_ignored_default_conf(self):
        ...
        node = self.nodes[0]
        (default_datadir / "BGL.conf").write_text(f"datadir={node.datadir_path}\n")
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "BGL.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_BGL.conf"
