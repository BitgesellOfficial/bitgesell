...
class ChainstateWriteCrashTest(BGLTestFramework):
    def set_test_params(self):
        ...
        # long-running test
        self.base_args = ["-limitdescendantsize=0", "-maxmempool=0", "-rpcservertimeout=900", "-dbbatchsize=20000"]
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If BGLd exits, then try again.  wait_for_node_exit()
                # should raise an exception if BGLd doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, BGLd isn't coming back up on restart.  Could be a
        # bug in BGLd, or we've gotten unlucky with our dbcrash ratio --
    ...
    def run_test(self):
        ...
        utxo_list = []
        for _ in range(5):
            utxo_list.extend(self.wallet.send_self_transfer_multi(from_node=self.nodes[3], num_outputs=100)['new_utxos'])
