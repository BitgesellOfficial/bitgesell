...
Verify that a BGLd node can load multiple wallet files
"""
...
class MultiWalletTest(BGLTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(w5_info['immature_balance'], 200)
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another BGLd?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another BGLd?", self.nodes[0].loadwallet, wallet_names[0])
        else:
