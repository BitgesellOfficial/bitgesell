...
class InitTest(BGLTestFramework):
    """
    ...
    def init_pid_test(self):
        BGL_PID_FILENAME_CUSTOM = "my_fancy_BGL_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = BGL_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / BGL_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / BGL_PID_FILENAME_CUSTOM
