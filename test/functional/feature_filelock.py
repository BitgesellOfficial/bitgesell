...
from test_framework.test_node import (
    BGL_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(BGLTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second BGLd instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second BGLd using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second bitcoind instance shutdown
        pid_file = datadir / BGL_PID_FILENAME_DEFAULT
