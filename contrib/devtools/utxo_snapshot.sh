...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <BGL-cli-call ...>'
  echo
...
if [[ "${OUTPUT_PATH}" = "-" ]]; then
  ...
  ${BGL_CLI_CALL} gettxoutsetinfo | grep hash_serialized_2 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
...
${BGL_CLI_CALL} reconsiderblock "${PIVOT_BLOCKHASH}"
