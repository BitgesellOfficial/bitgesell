...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing BGL-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='BGL-cli', help='BGL-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitgesell address to which the faucet should send')
...
parser.add_argument('BGL_cli_args', nargs='*', help='Arguments to pass on to BGL-cli (default: -signet)')

args = parser.parse_args()

if args.BGL_cli_args == []:
    args.BGL_cli_args = ['-signet']


def BGL_cli(rpc_command_and_params):
    argv = [args.cmd] + args.BGL_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = BGL_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = BGL_cli(['getnewaddress', 'faucet', 'bech32'])
