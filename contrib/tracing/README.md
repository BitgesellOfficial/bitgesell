...
Tracing (USDT) support for Bitgesell on Linux. For more information on
USDT support in Bitgesell see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `BGLd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `BGLd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Bitgesell USDT documentation] on how to list available tracepoints in your
`BGLd` binary.

[Bitgesell USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/BGLd
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/BGLd
```
...
In a different terminal, starting Bitgesell in SigNet mode and with
re-indexing enabled.

```
$ ./src/BGLd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/BGLd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/BGLd
```
