...
container=`docker run --memory=10g --cpus=4 -dit -e TZ='Etc/UTC' -e DEBIAN_FRONTEND='noninteractive' ubuntu:22.04 `
trap finish EXIT

docker exec $container apt-get -y update
docker exec $container apt-get -y install build-essential apt-utils devscripts equivs sudo cmake ninja-build #dpkg-dev
...
docker cp ../bitgesell_0.1.14.dsc $container:/root/repo/
docker cp ../bitgesell_0.1.14.tar.xz $container:/root/repo/
...
docker exec -w /home/user/build/bitgesell-0.1.14 $container sudo -u user debuild -b
docker cp $container:/home/user/build/bitgesell_0.1.14_amd64.deb bitgesell_0.1.14_amd64.deb
