...
CJDNS support was added to BGL Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `BGL-cli -addrinfo` to see the
...
You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known
to your node; run `BGL-cli help getnodeaddresses` for details.
