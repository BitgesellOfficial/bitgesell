# RapidCheck property-based testing for BGL Core

## Concept

Property-based testing is experimentally being added to BGL Core with
...
If RapidCheck is installed, BGL Core will automatically run the
...
3. Configure BGL Core with RapidCheck.

    `cd` to the directory of your local BGL repository and run
...
4. Build BGL Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_BGL-crypto_gen.o
    CXX      test/test_BGL-key_properties.o
    ```
...
That's it! You are now running property-based tests in BGL Core.
