...
- [Development guidelines](#development-guidelines)
    - [General BGL Core](#general-BGL-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BGL Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BGLs" on the test network, if you
...
To profile a running BGLd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep BGLd` -- sleep 60
```
...
BGL Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as BGL Core, where everyone uses
...
pay attention to for reviewers of BGL Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BGL Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of BGL Core, in which case changes should probably go
...
- src/univalue
  - Upstream at https://github.com/BGL-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with BGL's `select()` loop, because
...
compatibility between nodes. This happened in BGL 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BGL's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `BGL-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `BGL-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BGL Core, the wallet was always
