# Fuzzing BGL Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BGL Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/BitgesellOfficial/bitgesell/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of BGL Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of upstream Bitcoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/BitgesellOfficial/bitgesell/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/BitgesellOfficial/bitgesell/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/BitgesellOfficial/bitgesell/tree/master/src/test/fuzz).
...
It is possible to specify `BGLd` arguments to the `fuzz` executable.
...
Every single pull request submitted against the upstream Bitcoin Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. The same corpus is useful for validating BGL Core because this repository shares much of the fuzzing harness structure.
...
# Fuzzing BGL Core using afl++

## Quickstart guide

To quickly get started fuzzing BGL Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing BGL Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing BGL Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the BGL Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as BGL
Core without having to write any custom fuzzing harness. The `BGLd` server
...
```sh
$ mkdir bitgesell-honggfuzz-p2p/
$ cd bitgesell-honggfuzz-p2p/
$ git clone https://github.com/BitgesellOfficial/bitgesell
$ cd bitgesell/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
# Fuzzing BGL Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing BGL Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
The upstream Bitcoin Core project participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoin-core)
...
That project generally tries to disclose vulnerabilities as soon as possible after they
