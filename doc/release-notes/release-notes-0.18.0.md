Bitgesell version 0.18.0 is now available from:
...
`/Applications/Bitgesell` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
recommended to use Bitgesell on unsupported systems.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bitgesell
...
`~bitcoin/.bitcoin`. This change makes Bitgesell more consistent with
...
- A new document introduces Bitgesell's BIP174 [Partially-Signed
  Bitgesell Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bitgesell from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bitgesell to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bitgesell knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bitgesell, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside Bitgesell
...
This section describes planned changes to Bitgesell that may affect
other Bitgesell software and services.

- Since version 0.16.0, Bitgesell’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bitgesell 0.20 (expected about
  a year after 0.18), Bitgesell will default to native segwit
  ...
  to bech32 addresses, and if the Bitgesell project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bitgesell 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bitgesell
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bitgesell
...
- A sub-project of Bitgesell now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bitgesell.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bitgesell's own implementation, although entropy
  gathered by Bitgesell is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bitgesell a
...
- On macOS, Bitgesell now opts out of application CPU throttling
...
- #11526 Visual Studio build configuration for Bitgesell (sipsorcery)
...
- #12775 Integration of property based testing into Bitgesell (Christewart)
