...
This tutorial will describe how to use two instances of BGL Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed Bitcoin Transactions](psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
## Overview
In this tutorial we have two hosts, both running BGL Core v25.0
...
```sh
[offline]$ ./src/BGL-cli -signet -named createwallet \
...
```sh
[offline]$ ./src/BGL-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/BGL-cli -signet -named createwallet \
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" getnewaddress

tbgl1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajk4z962
```
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" listunspent

[
  {
    "txid": "0f3953dfc3eb8e753cd1633151837c5b9953992914ff32b7de08c47f1f29c762",
    "vout": 1,
    "address": "tbgl1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajk4z962",
    "label": "",
...
1. Get a destination address for the transaction. In this tutorial we'll be sending funds to the address `tbgl1q9k5w0nhnhyeh78snpxh0t5t7c3lxdeg38eva0h`, but if you don't need the coins for further testing you could send the coins back to the faucet.
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" send \
              '{"tbgl1q9k5w0nhnhyeh78snpxh0t5t7c3lxdeg38eva0h": 0.009}' \
              | jq -r '.psbt' \
...
```sh
[offline]$ ./src/BGL-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/BGL-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/BGL-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/BGL-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/BGL-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/BGL-cli -signet -rpcwallet="watch_only_wallet" listtransactions
