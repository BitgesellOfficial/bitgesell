# User-space, Statically Defined Tracing (USDT) for BGL Core

BGL Core includes statically defined tracepoints to allow for more
...
**Note**: BGL Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BGL Core
...
Multiple tools can list the available tracepoints in a `BGLd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BGL Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/BGLd
stap net        outbound_message 0x0000000000107c05 /src/BGLd
stap validation block_connected  0x00000000002fb10c /src/BGLd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BGL Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/BGLd | grep NT_STAPSDT -A 4 -B 2
