...
!define MUI_FINISHPAGE_RUN_PARAMETERS $INSTDIR\BGL-qt.exe
...
OutFile ..\BGL-0.1.13.0-win64-setup.exe
...
VIAddVersionKey ProductVersion "0.1.13"
VIAddVersionKey CompanyName "${COMPANY}"
VIAddVersionKey CompanyWebsite "${URL}"
VIAddVersionKey FileVersion "0.1.13"
...
Section -Main SEC0000
    SetOutPath $INSTDIR
    SetOverwrite on
    File ..\..\binaries_windows\BGL-qt.exe
    File /oname=COPYING.txt ..\COPYING
    File /oname=readme.txt ..\doc\README_windows.txt
    SetOutPath $INSTDIR\daemon
    File ..\..\binaries_windows\BGLd.exe
    File ..\..\binaries_windows\BGL-cli.exe
    File ..\..\binaries_windows\BGL-tx.exe
    File ..\..\binaries_windows\BGL-wallet.exe
...
Section -post SEC0001
    ...
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\$(^Name).lnk" $INSTDIR\BGL-qt.exe
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\BGL Core (testnet, 64-bit).lnk" "$INSTDIR\BGL-qt.exe" "-testnet" "$INSTDIR\BGL-qt.exe" 1
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\Uninstall $(^Name).lnk" $INSTDIR\uninstall.exe
    !insertmacro MUI_STARTMENU_WRITE_END
    WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$(^Name)" DisplayName "$(^Name)"
    WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$(^Name)" DisplayVersion "0.1.13"
...
Section /o -un.Main UNSEC0000
    Delete /REBOOTOK $INSTDIR\BGL-qt.exe
